...
sub deserialize_abstract {
  ...
  if ($version < 3) {
    my $standard = $schema_class->new(db => $self->db)->{abstract_schema};
  ...
  my $new_schema = $schema_class->new(db => $self->db);
  $new_schema->{abstract_schema} = $thawed_hash;
  $new_schema->{schema}          = dclone($thawed_hash);
  $new_schema->_adjust_schema;
...
sub get_empty_schema {
  my ($self) = @_;
  my $schema_class = ref($self) || $self;
  my $empty_schema = $schema_class->new(db => $self->db);
  $empty_schema->{abstract_schema} = {};
  $empty_schema->{schema}          = {};
  return $empty_schema;
}
